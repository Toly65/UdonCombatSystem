...
public class UCS_MagBelt : UdonSharpBehaviour
{
    ...
    {
        float x = beltFlipped ? -Mathf.Abs(baseLocalScale.x) : Mathf.Abs(baseLocalScale.x);
        float y = baseLocalScale.y * avatarScaleMultiplier;
        float z = baseLocalScale.z;
    ...
    public override void OnAvatarEyeHeightChanged(VRCPlayerApi player, float prevEyeHeightAsMeters)
    {
        ...
        {
            FlipBelt(flipBelt);
            //scale the belt based on the avatar's eye height to maintain a consistent appearance across different avatar sizes
            Vector3 headPosition = player.GetTrackingData(VRCPlayerApi.TrackingDataType.Head).position;
            if (prevEyeHeightAsMeters > 0.01f && !float.IsNaN(headPosition.y) && !float.IsInfinity(headPosition.y))
            {
                avatarScaleMultiplier = Mathf.Max(0.01f, headPosition.y / prevEyeHeightAsMeters);
            }
            else
            {
                avatarScaleMultiplier = 1f;
